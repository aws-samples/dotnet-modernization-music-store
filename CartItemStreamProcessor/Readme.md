...
WORKDIR /src
COPY ["CartItemStreamProcessor.csproj", "CartItemStreamProcessor/"]
RUN dotnet restore "CartItemStreamProcessor/CartItemStreamProcessor.csproj"

WORKDIR "/src/CartItemStreamProcessor"
COPY . .
RUN dotnet build "CartItemStreamProcessor.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "CartItemStreamProcessor.csproj" \
...
Execute unit tests
```
    cd "CartItemStreamProcessor/test/CartItemStreamProcessor.Tests"
    dotnet test
```

Deploy application
```
    cd "CartItemStreamProcessor/src/CartItemStreamProcessor"
